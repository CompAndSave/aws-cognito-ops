...
class Cognito {
  ...
  //
  static setCookie(res, accessToken, refreshToken, contextPath) {
    let now = new Date(), cookies = [];
    if (typeof accessToken === "string") {
      cookies.push(`AWSCognito_accessToken=${accessToken}; Path=${contextPath}/; Expires=${(new Date(now.getTime() + Cognito.accessTokenExp * 1000)).toUTCString()}; HttpOnly`);
    }
    if (typeof refreshToken === "string") {
      cookies.push(`AWSCognito_refreshToken=${refreshToken}; Path=${contextPath}/; Expires=${(new Date(now.getTime() + Cognito.refreshTokenExp * 1000)).toUTCString()}; HttpOnly`);
    }
