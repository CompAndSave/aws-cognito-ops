...
class Cognito {
  ...
  //
  static signOut(res, contextPath = Cognito.defaultContextPath, domain = Cognito.defaultCookieDomain) {
    res.setHeader("Set-Cookie", `AWSCognito_accessToken=; Path=${contextPath}/; Domain=${domain}; Expires=${(new Date()).toUTCString()};`);
    // res.cookie('AWSCognito_accessToken', "", { maxAge: 0 });
    // res.cookie('AWSCognito_refreshToken', "", { maxAge: 0 });
    // res.cookie('AWSCognito_username', "", { maxAge: 0 });
    return true;
  }
