...
class Cognito {
  ...
  //
  static setCookie(res, accessToken, refreshToken) {
    let now = new Date(), cookies = [];
    if (typeof accessToken === "string") {
      cookies.push(`AWSCognito_accessToken=${accessToken}; Path=/; Expires=${(new Date(now.getTime() + Cognito.accessTokenExp * 1000)).toUTCString()}; HttpOnly`);
    }
    if (typeof refreshToken === "string") {
      cookies.push(`AWSCognito_refreshToken=${refreshToken}; Path=/; Expires=${(new Date(now.getTime() + Cognito.refreshTokenExp * 1000)).toUTCString()}; HttpOnly`);
    }
